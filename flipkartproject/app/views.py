...
def showpricerange(req):
    if req.method=="GET":
        ...
        if r1 is not None and r2 is not None and r1.isdigit() and r2.isdigit():
            ...
            allproducts=Product.objects.all()
            context={'allproducts0':allproducts}
            return render(req,'index.html',context)
...
def searchproduct(req):
    ...
    if query:
        ...
        if len(allproducts)==0:
            messages.error(req,"No Iteam Found!!!!")
        else:
            allproducts=Product.objects.all()
    ...
    if req.user.is_authenticated:
        try:
            ...
            for items in cartitems:
                ...
                paymentdata=Payment.objects.create(reciptid=receiptid,orderid=orderdata,userid=userid,totalprice=totalamount)
                paymentdata.save()
            print(orderid,receiptid)

            client = razorpay.Client(auth=("rzp_test_wH0ggQnd7iT3nB", "eZseshY3oSsz2fcHZkTiSlCm"))
            data = { "amount": totalamount*100, "currency": "INR", "receipt": str(reciptid) }
            ...
            # subject=f"Flipkart payment status for your order={oderid}"
            # msg=f"hii {userid} thank you for using our service {tottalamount}"
            # emailfrom= settings.EMAIL_HOST_USER
            # receiver=[userid]
            # send_mail(subject,msg,emailfrom,rereceiver)
